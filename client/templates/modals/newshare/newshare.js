//初始化
var newShareCreate = function (imageData) {
    var latLng = Geolocation.latLng();

    if (! latLng) {
      return;
    }
  
    console.log("Share.insert");
    console.log(latLng);
    Meteor.call('Share.insert', latLng,imageData);
};


//调用服务端 Baoxian.insert
var newShareInsert = function (imageData,cam) {
    var latLng = Geolocation.latLng();

    if (! latLng) {
      return;
    }
  
    console.log("Share.insert");
    console.log(latLng);
    Meteor.call('Share.insert', latLng,imageData,photo);
};


// 拍照
var shareDate;
var sharephoto= function (photo) {
  
  shareDate= "data:image/jpeg;base64,";
  
  console.log(photo);

  MeteorCamera.getPicture(function (error, data) {
  // we have a picture
  if (! error) {
      shareDate = data;

    }
  });
};




Template.newShare.created = function () {
  this.autorun(function () {
  this.subscription = Meteor.subscribe('share');
  Session.set('id', this.data); 	
  console.log(this.data);   
  //console.log(Session.get('id')); 
  }.bind(this));
};

Template.newShare.helpers({
  veh: function () {
  var mo= Share.findOne({_id: Session.get('id')});
   console.log('share mo：');
   console.log(mo);
   return mo;
  },
  exampleDoc: function () {
   var mo= Share.findOne();
   console.log(mo);
   return mo;
  }
});


Template.newShare.events({
  'click [data-action=up]': function (event, template) {
    IonModal.close();
    IonKeyboard.close();
  },
  'click [data-action=share]': function (event, template) {
    console.log('分享');
    sharephoto('share');
	
  },
  
  
  
  
  'click [data-action=save]': function (event, template) {
  
    event.preventDefault();

    var post_share = {
	  share:shareDate,
  };

  
   // 插入一
     console.log('post_share');
     console.log(post_share);
    // var groceriesId = Brxx.insert({postProperties});
    // //Items.insert({list: groceriesId, name: "Watercress"});
    // console.log(groceriesId);
	
    // 插入二
   Share.insert( post_share, function(error,result) {
      if (error) {
        // 向用户显示错误信息
        //alert(error.reason);
        console.log(error);
      } else {
        console.log('插入完成');
      // IonModal.close();
      // IonKeyboard.close();
      // this.subscription = Meteor.subscribe('brxx');
      // Session.set('id', this.data).bind(this);  
        IonModal.close();
        var id = result;
		    console.log(result);
		    Session.set('id', id);
    
      }
    });
	
  }
  
});

